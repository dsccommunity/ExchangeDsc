<#
    .SYNOPSIS
        Automated unit integration for DSC_ExchDatabaseAvailabilityGroup DSC Resource
...
[System.String] $script:DSCResourceFriendlyName = 'ExchDatabaseAvailabilityGroup'
    ...
    if ($null -ne $compAccount)
    {
        ...
        if ($exchangeInstalled)
        {
            ...
            Describe 'Test Creating and Modifying a DAG, adding DAG members, creating a DAG database, and adding database copies' {
                ...
                Get-Module DSC_Exch* | Remove-Module -ErrorAction SilentlyContinue

                Import-Module -Name (Join-Path -Path $script:moduleRoot -ChildPath (Join-Path -Path 'source' -ChildPath (Join-Path -Path 'DSCResources' -ChildPath (Join-Path -Path "DSC_ExchDatabaseAvailabilityGroupMember" -ChildPath "DSC_ExchDatabaseAvailabilityGroupMember.psm1"))))
                ...
                if (([System.String]::IsNullOrEmpty($secondDAGMember)) -eq $false)
                {
                    ...
                    Get-Module DSC_Exch* | Remove-Module -ErrorAction SilentlyContinue
                    Import-Module -Name (Join-Path -Path $script:moduleRoot -ChildPath (Join-Path -Path 'source' -ChildPath (Join-Path -Path 'DSCResources' -ChildPath (Join-Path -Path "DSC_ExchMailboxDatabase" -ChildPath "DSC_ExchMailboxDatabase.psm1"))))
                    ...
                    # Add DB Copy
                    Get-Module DSC_Exch* | Remove-Module -ErrorAction SilentlyContinue
                    Import-Module -Name (Join-Path -Path $script:moduleRoot -ChildPath (Join-Path -Path 'source' -ChildPath (Join-Path -Path 'DSCResources' -ChildPath (Join-Path -Path "DSC_ExchMailboxDatabaseCopy" -ChildPath "DSC_ExchMailboxDatabaseCopy.psm1"))))
