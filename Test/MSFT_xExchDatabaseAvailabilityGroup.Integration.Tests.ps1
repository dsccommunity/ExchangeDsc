...
function PrepTestDAG
{
    ...
    if ((Get-MailboxDatabase -Identity "$($TestDBName)" -ErrorAction SilentlyContinue) -ne $null)
    {
        Get-MailboxDatabaseCopyStatus -Identity "$($TestDBName)" | where {$_.MailboxServer -ne "$($TestServerName1)"} | Remove-MailboxDatabaseCopy -Confirm:$false
    }
    ...
    Write-Verbose "Finsihed cleaning up test DAG and related resources"
}
...
if ($exchangeInstalled)
{
    ...
    {
        $Global:SecondDAGMember = Read-Host -Prompt "Enter the name of the second DAG member to use for testing"
    }
